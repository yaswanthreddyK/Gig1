...
const ClientSettings = () => {
      ...
      setTimeout(() => {
        ...
        reader.onload = () => {
          setFormData((prevFormData)=>{
            return {
            ...prevFormData,
            logo: reader.result, // Update logo with the loaded image data
            isUploading: false,
          }
        });
        };
  ...
  const handleCancelUpload = () => {
    setFormData((prevFormData) => ({
      ...prevFormData,
      logo: '', // Reset logo to a defined value
      isUploading: false,
    }));
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log(formData); // Send formData to backend or perform other actions
  };
            ...
            {!formData.logo && (
              <label htmlFor="logo-input">
                <StyledLogoButton component="span" disabled={formData.isUploading}>
                  {formData.isUploading ? <CircularProgress size={24} sx={{color: "white"}} /> : 'Upload Logo'}
                </StyledLogoButton>
              </label>
            )}
            <StyledFileInput
